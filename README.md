...
package beginning with `4.0.0-rc.5`. It is a deliberately thin compatibility
...
`v4.0.0-rc.5` or its positive numbered append-only corrective form;
...
The RC.5 train starts from the canonical source tag recorded in
`release/version.json`. The `github-release` job requires an operator review
and a `v*` tag policy, while npm remains behind its trusted-publisher
environment. If a workflow-only correction is required before publication,
use the next positive `v4.0.0-rc.5-release.N` tag; never move an existing tag.
