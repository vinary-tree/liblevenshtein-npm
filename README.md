...
package beginning with `4.0.0-rc.3`. It is a deliberately thin compatibility
...
`v4.0.0-rc.3`; publication uses npm trusted publishing and an explicit
`--tag next`. Do not run `npm dist-tag add liblevenshtein@4.0.0-rc.3 latest`
...
After `@vinary-tree/liblevenshtein@4.0.0-rc.3` resolves and passes an
...
verify `next = 4.0.0-rc.3` and `latest = 2.0.4`; unlike the six new scoped
